...
MIN_MAX_TOKENS_WITH_TABLES = 1800
...
def ensure_citations(content: str, chunk_ids: list[str]) -> str:
    ...
    content = re.sub(r"(?:\n|^)[ \t]*资料来源：[^\n]*\s*$", "", content)
    # 输出被截断时会留下未闭合的 [ref:...；先移除，否则下面按无效引用清洗时
    # 会跨行匹配到后续追加的表格内容并整块误删
    content = re.sub(r"\[ref:[^\]\n]*$", "", content, flags=re.MULTILINE)

    known_ids = set(chunk_ids)
    used_ids = set(re.findall(r"\[ref:([^\]\n]+)\]", content))
...
def truncation_warning(usage: dict | None, max_tokens: int) -> str | None:
    completion_tokens = (usage or {}).get("completion_tokens")
    if isinstance(completion_tokens, int) and completion_tokens >= max_tokens:
        return f"正文输出已达 max_tokens={max_tokens} 上限，内容可能被截断，建议增大生成长度后重试"
    return None
...
def prepare_section_base(conn, section_id: str, payload: GenerateRequest) -> dict:
    ...
    if payload.tables:
        ...
        # 带表格生成时正文还要输出占位符，过低的 max_tokens 极易截断
        if payload.max_tokens < MIN_MAX_TOKENS_WITH_TABLES:
            payload.max_tokens = MIN_MAX_TOKENS_WITH_TABLES
    ...
    for table_item in (table_metadata or {}).get("tables", []):
        ...
        record_ai_call(
            ...
            metadata={
                ...
                "table_query": table_item.get("table_query"),
                "retrieved_chunk_ids": table_item.get("retrieved_chunk_ids") or [],
    ...
    with get_connection() as conn:
        try:
            ...
            yield format_sse(
                ...
                {
                    "phase": "generating",
                    "message": f"已命中 {len(retrieved_chunks)} 个资料片段，模型生成中…",
                },
            )
            ...
            try:
                for delta in llm_client.iter_generate(
                    ...
                    yield format_sse("delta", {"content": delta.text})
            ...
            truncated_warning = truncation_warning(llm_client.last_usage, payload.max_tokens)
            if truncated_warning:
                yield format_sse(
                    "status",
                    {"phase": "truncated", "message": truncated_warning},
                )
            ...
            if truncated_warning:
                table_metadata.setdefault("table_warnings", []).insert(0, truncated_warning)
...
def generate_section(section_id: str, payload: GenerateRequest):
    ...
    with get_connection() as conn:
        ...
        truncated_warning = truncation_warning(llm_result.usage, payload.max_tokens)
        if truncated_warning:
            table_metadata.setdefault("table_warnings", []).insert(0, truncated_warning)
