...
- 健康检查与运行时配置
- 报告模板与章节状态（SQLite）
- 参考资料上传、解析（PDF 表格用 pdfplumber 抽取）、分块、向量化与 Chroma 检索
- 章节生成（同步 / SSE 流式），模型为通义 `qwen3.6-35b-a3b`；正文达到 `max_tokens` 上限时输出截断警示
- 表格流水线（`table_service.py`）：AI 汇总（按表格描述独立检索 + 畸形 JSON 按位重排修复）与引用原表
- 引用规范化（含未闭合 / 无效 `[ref:…]` 清洗）与章节版本落库
- 模拟导出记录
